...
Game::Game():
    ...
    m_levelFiles.push_back("assets/map.tmx");
    ...
    if (SDL_Init(SDL_INIT_EVERYTHING) == 0) {
        ...
        if (m_pWindow != 0) {  // window init success
            ...
            if (m_pRenderer != 0) {
                ...
                // Keep the game's design aspect ratio, but scale it up and
                // center it in the window with letterbox/pillarbox bars
...
#ifdef __ANDROID__
    // Touch controls are positioned in real screen pixel coordinates,
    // not the game's logical design resolution, so drop the logical
    // size scaling before drawing them, then restore it for next frame.
    SDL_RenderSetLogicalSize(m_pRenderer, 0, 0);
    //TheInputHandler::Instance()->renderTouchControls(m_pRenderer);
    SDL_RenderSetLogicalSize(m_pRenderer, m_gameWidth, m_gameHeight);
#endif
