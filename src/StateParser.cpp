...
bool StateParser::parseState(const char *stateFile, string stateID, vector<GameObject*> *pObjects, std::vector<std::string> *pTextureIDs) {
    ...
    // load the state file
    // (loaded via SDL_RWops rather than TiXmlDocument::LoadFile so this
    // also works when stateFile lives inside an Android APK's assets)
    if (!LoadXMLFromAssets(xmlDoc, stateFile)) {
        return false;
    }
    ...
    // get this states root node and adding it to pStateRoot
